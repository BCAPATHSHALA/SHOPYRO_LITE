...
import { startTransition, useMemo, Suspense } from "react";
...
function VariantOptionSelectorContent({
  option,
  variant,
  product,
...
export function VariantOptionSelector(props: VariantOptionSelectorProps) {
  return (
    <Suspense
      fallback={
        <dl
          className={variantOptionSelectorVariants({ variant: props.variant })}
        >
    ...
    >
      <VariantOptionSelectorContent {...props} />
    </Suspense>
  );
}

function useSelectedVariantContent(product: Product) {
  const { variants, options } = product;
  const searchParams = useSearchParams();
...
export const useSelectedVariant = (product: Product) => {
  return useSelectedVariantContent(product);
};
